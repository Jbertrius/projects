...
function buildDashboard(source) {
  ...
  const memberIndex = Object.fromEntries(members.map((member) => [member.id, member]));
  const memberMeetingGroups = groupBy(meetings, (meeting) => meeting.memberId || meeting.memberName || "unknown");
  ...
  return {
    ...
    monthlyMeetings,
    members: memberSummaries.slice(0, 12),
    formationTimeline,
    pipeline: [
