...
function normalizeStudentDedupKey(value) {
  return String(value || "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/\s+/g, " ")
...
function parseAttendanceBlock(text, lessonDate = "") {
  ...
  const seenRegisteredEntries = new Set();
  const seenUnregisteredEntries = new Set();
  ...
  for (const rawLine of String(text || "").split("\n")) {
    ...
    if (inNonRegistered) {
      const dedupeKey = normalizeStudentDedupKey(student.name);
      if (!dedupeKey || seenUnregisteredEntries.has(dedupeKey)) {
        continue;
      }
      seenUnregisteredEntries.add(dedupeKey);
    ...
    const dedupeKey = `${normalizeStudentDedupKey(student.name)}|${String(student.status || "unknown").toLowerCase()}|${normalizeSectionLabel(currentGroup)}`;
    if (!dedupeKey.startsWith("|") && !seenRegisteredEntries.has(dedupeKey)) {
      seenRegisteredEntries.add(dedupeKey);
