...
function parseClassHeader(line) {
  ...
  const attendanceMatch = normalized.match(/attendance\s*-\s*([^-]+?)\s*-\s*(.+)$/iu);
  if (attendanceMatch) {
    return {
      classCode: normalizeText(attendanceMatch[1]),
      churchName: cleanChurchName(attendanceMatch[2])
    };
  }

  const classOpenMatch = normalized.match(/classe\s+ouverte\s*-\s*([^-]+?)\s*-\s*(.+)$/iu);
  if (classOpenMatch) {
    return {
      classCode: normalizeText(classOpenMatch[1]),
      churchName: cleanChurchName(classOpenMatch[2])
    };
  }
