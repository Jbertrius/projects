...
async function createAcademyLessonRecord(parsed) {
  ...
  const classStudentIds = registeredStudentRefsWithReg
  ...
  for (const student of registeredStudentRefsWithReg) {
...
async function updatePastorInFirestore(input) {
  ...
  const payload = {
    ...
    academy_class: String(input.academy_class || "").trim(),
    class_number: String(input.class_number || "").trim(),
    cell_number: String(input.cell_number || "").trim(),
    current_mission: String(input.current_mission || "").trim(),
