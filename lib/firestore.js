...
async function createAcademyLessonRecord(parsed) {
  ...
  for (const studentName of unregisteredStudents) {
    ...
    await writeDocument(
      ...
      buildAcademyUnregisteredId(lessonId, studentName),
      toAcademyUnregisteredDocument({
        id: buildAcademyUnregisteredId(lessonId, studentName),
        ...
        lesson_id: lessonId,
        lesson_title: parsed.lesson_title,
        note: String(parsed.absence_notes?.[studentName] || "").trim(),
        timestamp: utcTimestamp()
      }),
      accessToken
    );
...
async function listAcademyLessonsForClass(classId) {
  ...
  const docs = await listCollectionDocuments("academyLessons", 2000);
  return docs
    .map((doc) => {
      const { id, fields } = parseFirestoreDocument(doc);
      return {
...
async function listAcademyAttendanceForClass(classId) {
  const cid = String(classId || "").trim();
  const docs = await listCollectionDocuments("academyAttendance", 3000);
  return docs
    .map(mapAcademyAttendanceDocument)
    .filter((r) => r.class_id === cid);
}
...
async function listAcademyAttendanceForStudent(studentName) {
  ...
  return docs
    .map(mapAcademyAttendanceDocument)
    .filter((r) => r.student_name.toLowerCase() === target)
